...
```bash
go get github.com/PlatformCore/libpackage/middleware/http
```
...
func main() {
    ...
    chain := httpmw.New(
        ...
        httpmw.WithRecovery(middleware.RecoveryOptions{Logger: logger}),
        httpmw.WithSecurity(),
        httpmw.WithCORS(middleware.CORSConfig{AllowedOrigins: []string{"*"}}),
        httpmw.WithCompress(),
        httpmw.WithTracing(middleware.TracingConfig{ServiceName: "my-service"}),
        httpmw.WithMetrics(middleware.MetricsConfig{}),
        httpmw.WithAuth(middleware.AuthConfig{
            HMACSecret: []byte("my-secret"),
        }),
        httpmw.WithRateLimit(middleware.RateLimitConfig{
            RequestsPerSecond: 100,
            Burst:             200,
        }),
        httpmw.WithTimeout(middleware.TimeoutConfig{Timeout: 30 * time.Second}),
    )

    mux := http.NewServeMux()
    httpmw.WithHealth(middleware.HealthConfig{}, mux)
...
id := httpmw.GetRequestID(r.Context())

// Get validated JWT claims.
claims := httpmw.GetClaims(r.Context())
fmt.Println(claims.UserID, claims.Roles, claims.TenantID)

// Get a validated + decoded request body.
body, ok := httpmw.GetValidatedBody[CreateUserRequest](r.Context())
```
...
// Register:
chain.Append(httpmw.WithBodyValidator[CreateUserRequest](middleware.ValidatorConfig{}))
```
