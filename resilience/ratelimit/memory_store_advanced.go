...
// MemoryStore is a direct, in-package store for the core Limiter. It supports
// fixed-window, sliding-window, and token-bucket evaluation without requiring
// compat/goratelimit or any wrapper package.
...
func (s *MemoryStore) evalFixed(key string, item memoryItem, p Policy, now time.Time) StoreResponse {
	...
	s.items[key] = item
	used := item.count
	return StoreResponse{Used: used, Limit: p.Limit, Remaining: max64(0, p.Limit-used), ResetAt: item.resetAt, Allowed: used <= p.Limit, Metadata: map[string]string{"mode": string(StrategyFixedWindow)}}
}
...
func (s *MemoryStore) evalSliding(key string, item memoryItem, p Policy, now time.Time) StoreResponse {
	...
	return StoreResponse{Used: used, Limit: p.Limit, Remaining: max64(0, p.Limit-used), ResetAt: now.Add(p.Window), Allowed: used <= p.Limit, Metadata: map[string]string{"mode": string(StrategySlidingWindow)}}
}
...
func (s *MemoryStore) evalTokenBucket(key string, item memoryItem, p Policy, now time.Time) StoreResponse {
	...
	return StoreResponse{Used: p.Burst - remaining, Limit: p.Burst, Remaining: remaining, ResetAt: now.Add(time.Second), Allowed: allowed, Metadata: map[string]string{"mode": string(StrategyTokenBucket)}}
}
