// Package retry provides a production-grade retry engine with exponential
...
const (
	...
	BackoffDecorrelated                        // AWS-style decorrelated jitter cap
)
...
func calcDelay(cfg Config, attempt int) time.Duration {
	...
	case BackoffDecorrelated:
		...
		prev := time.Duration(float64(base) * math.Pow(mult, float64(maxInt(1, attempt-1))))
		upper := prev * 3
		if upper < base {
			upper = base
		}
		delay = base + time.Duration(rand.Float64()*float64(upper-base))
		if delay > max {
			delay = max
		}
	default: // Exponential
