...
var ErrTimedOut = errors.New("timeout: operation timed out")

// ErrFallbackFailed is returned when both the primary and fallback fail.
var ErrFallbackFailed = errors.New("timeout: primary and fallback both failed")
...
func Guard(ctx context.Context, grace time.Duration, fn func()) {
	...
	case <-done:
	case <-timer.C:
		panic(fmt.Sprintf("timeout: goroutine leaked past deadline+grace (%s)", grace))
	}
}
